...
import { TasksOverviewChart } from '@/components/charts/TasksOverviewChart';
import { BoardsComparisonChart } from '@/components/charts/BoardsComparisonChart';
import { PriorityChart } from '@/components/charts/PriorityChart';
import { ActivityChart } from '@/components/charts/ActivityChart';
import { TasksByStatusChart } from '@/components/charts/TasksByStatusChart';
import { AverageTimeInColumnChart } from '@/components/charts/AverageTimeInColumnChart';
...
export default function Home() {
  ...
  const filteredTasks = useMemo(() => {
    ...
    if (selectedBoard !== 'all') {
      filtered = filtered.filter((t) => t.boardId === selectedBoard);
    }
    ...
    if (dateRange.end) {
      ...
      filtered = filtered.filter((t) => t.createdAt <= endDate.toISOString());
    }
  ...
  const stats = useMemo(() => {
    const totalTasks = filteredTasks.filter((t) => t.status !== 'archive').length;
    const completedTasks = filteredTasks.filter((t) => t.status === 'done').length;
    const inProgressTasks = filteredTasks.filter(
      (t) => t.status !== 'todo' && t.status !== 'done' && t.status !== 'archive'
    ).length;
  ...
  const productivityMetrics = useMemo(() => {
    ...
    const completedLast7Days = filteredTasks.filter((t) => 
    ...
    const velocity = tasksWithEstimates.length > 0 
      ? tasksWithEstimates.reduce((sum, t) => sum + (t.stimatedTime || 0), 0) / tasksWithEstimates.length
      : undefined;
  ...
  const priorityData = useMemo(() => {
    const activeTasks = filteredTasks.filter((t) => t.status !== 'archive');
    return {
  ...
  const averageTimeInColumn = useMemo(() => {
    ...
    return boardColumns.map((col) => {
      ...
      const totalTime = tasksInColumn.reduce((sum, task) => {
  ...
  const activityData = useMemo(() => {
    ...
    return last7Days.map((date) => {
      const created = filteredTasks.filter((t) => t.createdAt?.startsWith(date)).length;
      const completed = filteredTasks.filter((t) => t.completedAt?.startsWith(date)).length;
